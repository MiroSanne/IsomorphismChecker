...
def count_forest_isomorphisms(graphs: list[Graph]):
    ...
    trees_G = [building_subgraph(G, C) for C in components_G]
    trees_H = [building_subgraph(H, C) for C in components_H]
    roots_G = [finding_root_tree(T) for T in trees_G]
    roots_H = [finding_root_tree(T) for T in trees_H]
    if sorted(len(r) for r in roots_G) != sorted(len(r) for r in roots_H):
        return 0

    AHU_code_Forest_G = [sorted([AHU(r) for r in R]) for R in roots_G]
    AHU_code_Forest_H = [sorted([AHU(r) for r in R]) for R in roots_H]
...
def counting_trees(trees: list[list[str]]):
    """
    Count occurrences of each unique tree structure.

    Input: list of tree AHU code lists.
    Output: dict mapping each unique tree key to its count.
    """
    counts = {}
    for t in trees:
        key = tuple(sorted(t))
